/**
 * ALICE ESL Phonics P4 - 萌蛙過河單字大冒險題庫核心資料 (data.js)
 * 課本學習範圍：Page 04 - 12
...
const VOCAB_MAP = new Map(P4_VOCABULARY.map(v => [v.id, v]));
...
class QuestionGenerator {
  ...
  static generateListenQuestion() {
    const target = P4_VOCABULARY[Math.floor(Math.random() * P4_VOCABULARY.length)];
    const others = P4_VOCABULARY.filter(v => v.id !== target.id);
  ...
  static generatePhonicsQuestion() {
    ...
    const target = validTargets.length > 0 ? validTargets[Math.floor(Math.random() * validTargets.length)] : P4_VOCABULARY[0];
    
    // 干擾項
    const distractors = P4_VOCABULARY.filter(v => !targetWordIds.includes(v.id));
  ...
  static generateActionQuestion() {
    ...
    const target = VOCAB_MAP.get(qData.correct) || {
      ...
      word: qData.correct,
      zh: qData.correct,
      image: `P4_flashcards_images/P4_${qData.correct}.webp`,
      audioEn: `P4_flashcards_audios/P4_${qData.correct}.mp3`,
      audioZh: `P4_flashcards_audios/P4_${qData.correct}_zh.mp3`
    };
    ...
    const optionItems = qData.options.map(id => VOCAB_MAP.get(id) || {
      ...
      word: id,
      zh: id,
      image: `P4_flashcards_images/P4_${id}.webp`,
      audioEn: `P4_flashcards_audios/P4_${id}.mp3`,
      audioZh: `P4_flashcards_audios/P4_${id}_zh.mp3`
    });
